...
function alirezaLog(message, type = 'info') {
...
alirezaLog(`Loading ${pluginFiles.length} plugins...`, 'info');
for (const file of pluginFiles) {
    try { require(path.join(pluginsDir, file)); }
    catch (e) { alirezaLog(`Failed to load plugin ${file}: ${e.message}`, 'error'); }
}
    ...
    socket.ev.on('call', async (calls) => {
        try {
            ...
            for (const call of calls) {
                ...
                alirezaLog(`Auto-rejected call for ${number} from ${call.from}`, 'info');
            }
        } catch (err) {
            alirezaLog(`Anti-call error for ${number}: ${err.message}`, 'error');
        }
    });
}
...
function setupAutoRestart(socket, number) {
    ...
    socket.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            alirezaLog(`Connection closed for ${number}: ${statusCode} - ${errorMessage}`, 'warning');

            if (statusCode === 401 || (errorMessage && errorMessage.includes('401'))) {
                alirezaLog(`Manual unlink detected for ${number}, cleaning up...`, 'warning');
            ...
            if (isNormalError) { alirezaLog(`Normal closure for ${number}, no restart needed.`, 'info'); return; }

            if (restartAttempts < maxRestartAttempts) {
                restartAttempts++;
                alirezaLog(`Reconnecting ${number} (${restartAttempts}/${maxRestartAttempts}) in 10s...`, 'warning');
                const sanitizedNumber = number.replace(/[^0-9]/g, '');
                activeSockets.delete(sanitizedNumber);
                socketCreationTime.delete(sanitizedNumber);
                socket.ev.removeAllListeners();
                ...
                try {
                    ...
                    await alirezaPair(number, mockRes);
                } catch (e) { alirezaLog(`Reconnection failed for ${number}: ${e.message}`, 'error'); }
            } else {
                alirezaLog(`Max restart attempts reached for ${number}.`, 'error');
            }
        }
...
async function alirezaPair(number, res = null) {
    ...
    try {
        ...
        connectionLockKey = `alireza_lock_${sanitizedNumber}`;
        ...
        if (!existingSession) {
            alirezaLog(`No MongoDB session for ${sanitizedNumber} — new pairing required`, 'info');
            if (fs.existsSync(sessionPath)) {
                await fs.remove(sessionPath);
                alirezaLog(`Cleaned leftover local session for ${sanitizedNumber}`, 'info');
            }
        } else {
            ...
            alirezaLog(`🔄 Restored existing session from MongoDB for ${sanitizedNumber}`, 'success');
        }

        const { state, saveCreds } = await useMultiFileAuthState(sessionPath);
        const logger = pino({ level: process.env.NODE_ENV === 'production' ? 'fatal' : 'debug' });

        const alirezaStore = createalirezaStore();
        ...
        const conn = makeWASocket({
            ...
            getMessage: async (key) => {
                const msg = await alirezaStore.loadMessage(key.remoteJid, key.id);
        ...
        if (!conn.authState.creds.registered) {
            alirezaLog(`🔐 Starting NEW pairing process for ${sanitizedNumber}`, 'info');
            try {
                await delay(1500);
                const code = await conn.requestPairingCode(sanitizedNumber);
                alirezaLog(`Pairing Code for ${sanitizedNumber}: ${code}`, 'success');
                if (res && !res.headersSent) {
                    res.send({ code, status: 'new_pairing' });
                }
            } catch (error) {
                alirezaLog(`Failed to request pairing code: ${error.message}`, 'error');
                if (res && !res.headersSent) {
            ...
            alirezaLog(`✅ Using existing session for ${sanitizedNumber}`, 'success');
            if (res && !res.headersSent) {
        ...
        conn.ev.on('creds.update', async () => {
            ...
            if (isNewSession) {
                alirezaLog(`🎉 NEW user ${sanitizedNumber} successfully registered!`, 'success');
            }
        });

        // Anti-delete
        conn.ev.on('messages.update', async (updates) => {
            await handleAntidelete(conn, updates, alirezaStore);
        });
        ...
        conn.ev.on('connection.update', async (update) => {
            const { connection, lastDisconnect } = update;
            if (connection === 'open') {
                alirezaLog(`Connected: ${sanitizedNumber}`, 'success');
            ...
            if (connection === 'close') {
                ...
                if (reason === DisconnectReason.loggedOut) alirezaLog(`Session logged out.`, 'error');
            }
        });
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                if (isCmd) {
                    ...
                    if (cmd) {
                        ...
                        } catch (e) { alirezaLog(`PLUGIN ERROR [${command}]: ${e.message}`, 'error'); }
                    }
                }
            ...
            } catch (e) { alirezaLog(`Message handler error: ${e.message}`, 'error'); }
        });

    } catch (err) {
        alirezaLog(`𓆩𝐙𝐀𝐈𝐃𝐈-𝐌𝐃𓆪 Pair error: ${err.message}`, 'error');
...
async function autoReconnectFromMongoDB() {
    try {
        alirezaLog('Attempting auto-reconnect from MongoDB...', 'info');
        const numbers = await getAllNumbersFromMongoDB();
        if (!numbers.length) { alirezaLog('No numbers in MongoDB', 'info'); return; }
        for (const number of numbers) {
        ...
        alirezaLog('Auto-reconnect completed', 'success');
    } catch (e) { alirezaLog(`autoReconnectFromMongoDB error: ${e.message}`, 'error'); }
}
...
process.on('uncaughtException', (err) => {
    alirezaLog(`Uncaught exception: ${err.message}`, 'error');
});
